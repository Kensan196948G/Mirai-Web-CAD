...
破棄(Superseded by [ADR-0002](ADR-0002-dwg-scope-drop-dxf-only.md)、2026-08-30)

> 本ADRはDWG対応の選定を検討したが、2026-08-30付ADR-0002によりDWG対応自体を見送る決定がなされたため本ADRは破棄された。以下は当時の検討記録として保持する。
...
> **本補遺は無効な履歴記録である(2026-08-30、ADR-0002により失効)。** 以下の記述は当時の暫定方針であり、以後のいかなる作業の指示としても採用しない。特に「保留解除条件」欄が示す`file.originalDwgVersion`フィールドと新規ADR-0002起票の指示は無効である。実際には`file.originalDwgVersion`フィールドは[ADR-0002](ADR-0002-dwg-scope-drop-dxf-only.md)によりDWG対応自体が恒久的に対象外となった結果、台帳スキーマから削除済みであり(2026-08-30、本補遺が起票を予告したADR-0002とは異なる決定内容)、再登録してはならない。以下は当時の検討経緯としてのみ保持する。

80-90%代替方針Phase 0(代替率測定基盤)の着手時点で、以下を確定する(当時の記録、提案中ステータスを反映)。ODA File Converter商用ライセンス取得は依然として未取得・未検討であり、DWG対応の実装自体は見送る、としていた。
