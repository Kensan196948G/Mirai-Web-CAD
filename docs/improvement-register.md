...
| P0-22 | 閲覧者ロールのUIボタン活性化制御 | 全利用者/権限UXの明確化 | 80-90%方針§12直近アクション#1「閲覧者でも新規、保存、作図、移動、削除等のボタンが有効」の是正 | 低 1日 | P0 | ROLE_POLICIES.canEdit/canApprove | 閲覧者ロールで新規図面/保存/開く/作図・修正・注釈系リボンボタンがdisabled、選択・計測・表示系は引き続き有効 | 完了(2026-08-30): `src/app.js`の`ribbonButtonHtml`にtool/act種別からcanEdit/canApprove要否を判定する`ribbonButtonDisabled`を追加。`newDrawingBtn`/`importBtn`/`quickSaveBtn`にも同条件を適用。サーバー側のfail-closed(P0-14)は既存のまま無変更、UI層の追加防御。E2E回帰テストを更新(disabled状態の検証) |
| P0-23 | A4/A3レイアウトプレビューの未保存不整合 | CAD担当/レイアウト作業の信頼性 | 80-90%方針§12直近アクション#3「A4選択とA3横プレビューが同時に表示される場合がある」の是正 | 低 0.5日 | P0 | `#layoutForm`のinput/change | 用紙サイズ変更が「設定保存」を押す前でもプレビュー(サイズ・ラベル)へ即時反映される | 完了(2026-08-30): `previewLayoutFromForm`を追加し`#layoutForm`の`input`/`change`イベントでプレビューのみ即時更新(実際のコミットは引き続き「設定保存」submit時)。回帰E2Eテスト追加 |
