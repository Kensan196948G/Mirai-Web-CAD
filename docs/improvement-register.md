...
| P0-09 | 合成監視・通知 | IT/DX 7名 | 発見時間短縮 | 中 2日 | P0 | 通知先 | 5分監視、health/demo/write境界alert | 一部完了: healthにstatus/version/timestamp/503を実装(2026-08-27)。GitHub Actions 15分間隔の合成監視+`incident`Issue自動起票+復旧時自動close、Webhook通知(`MONITOR_WEBHOOK_URL`任意設定)を実装(2026-08-29)。schedule実行間隔はGitHub仕様上の保証なし、当番表・重大度別SLA・Cloudflare/Neon内部5xx相関は未着手 |
...
| P0-12 | 本番バックアップ自動化(枠組み) | 運用/RPO改善 | 日次archiveの機械実行 | 低 1日 | P0 | `PRODUCTION_DATABASE_URL` Secret投入(人間承認) | 日次workflow成功、Secret未設定時はops Issueでスキップ通知 | 一部完了(2026-08-29): workflow実装済み。Secret未投入のため実運用は未開始 |
