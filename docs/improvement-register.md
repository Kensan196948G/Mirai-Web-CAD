...
| P0-29 | 設定ダイアログにAIモデル接続設定(実験的)を追加 | 全利用者/将来のAI連携準備(ユーザー要望) | ユーザー要望「AIモデルAPIトークン設定も出来るようにしてください」。ユーザーが伏せ字入力・テスト/保存/クリアボタンを明示指定し、ブラウザ保存のみ・サーバー送信なしのセキュリティリスクを理解した上で実装を承認 | 中 1日 | P2 | 現状AI機能は外部LLM未使用(ルールベース) | APIキー欄がtype="password"、プロバイダ選択(OpenAI/Anthropic/カスタム)、接続テスト/保存/クリアがLocalStorageのみで完結し再読込後も保持される | 完了(2026-08-30): `state.aiSettings`(LocalStorage key`mirai-web-cad-ai-settings`、既存設定とは分離)を新設。設定ダイアログに警告文(サーバー送信なし・共有端末非推奨を明記)、プロバイダ選択、APIキー入力(伏せ字)、接続テスト/保存/クリアボタンを追加。接続テストは選択プロバイダへ直接fetchを試みるが、多くのプロバイダはCORSでブロックするため失敗時はその旨を案内。現状のAI提案機能(buildAiProposal)自体はルールベースのまま無変更、このキーは接続テスト以外に使用しない。回帰テスト2件追加 |
