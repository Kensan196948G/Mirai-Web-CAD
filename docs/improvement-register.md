...
| P0-30 | `/src/*`のエッジキャッシュがデプロイ後も古いJS/CSSを配信 | 全利用者/デプロイ即時性 | ユーザー報告「変わったように見えません」。UI改善(P0-27〜29)を本番反映したが、Cloudflareのエッジ(CDN)がオリジンのCache-Control欠如を拡張子ベースの既定TTL(4時間)で補完し、デプロイ前の`styles.css`(最大約2時間遅れ)を配信し続けていた | 低 0.2日 | P0 | Cloudflare Cache Level既定挙動 | `/src/*`がCloudflareの既定TTLに委ねられず、デプロイ後は毎回オリジンへ再検証される | 完了(2026-08-30): 緊急対応としてCloudflare API(`purge_cache`)で該当URLを強制パージし即時解消。恒久対応として`_headers`に`/src/* → Cache-Control: no-cache, must-revalidate`を追加(オリジン発のCache-Controlが無かったことが根本原因、コード側のバグではなくCloudflare既定挙動と判明)。`/assets/*`のimmutableキャッシュ、ルート`/`(DYNAMIC扱い)は無変更。回帰テスト`tests/http-bridge.test.js`新規追加(3件) |
