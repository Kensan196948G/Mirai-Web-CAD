...
- 投資判断: **条件付き継続**。Cloudflare Accessの複数利用者展開・Entra ID連携(Issue #5)、DWG往復、寸法・レイアウト・PDF、案件単位RBACを継続条件とする。バックアップのオフサイト化、ソーク運用後のCloudflare Pages関連ファイル削除・Neonプロジェクト削除(人間実施)も残課題
