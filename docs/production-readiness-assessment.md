...
| 評価軸 | 改善前 | 8/26後 | 8/27後 | 根拠と残差 |
| --- | ---: | ---: | ---: | --- |
| 業務適合性 | 32 | 34 | 34 | 土木デモ、数量、レビューあり。案件、工種、成果品、発注者様式なし |
| 機能完成度 | 24 | 25 | 25 | 基本図形と編集の一部。寸法、ハッチ、ブロック、外部参照、レイアウト、DWG/PDFなし |
| UI/UX | 48 | 52 | 52 | CAD配置とCLIあり。公開閲覧の権限表示を固定。大量操作・プロパティ編集は不足 |
| アクセシビリティ | 70 | 71 | 71 | axe、キーボード、モバイルE2Eあり。Canvas図形の代替表現は不足 |
| データ品質 | 38 | 44 | 44 | schema検証、重複ID検査、hash、明示的公開属性。座標系・単位変換・系譜監視なし |
| AI有効性 | 22 | 22 | 22 | 人間承認と差分あり。外部AI/RAGではなく、土木判断を支援する根拠データなし |
| 設計 | 45 | 51 | 51 | コマンドモデル、API/Store分離。更新原子化を追加。案件境界と非同期処理なし |
| コード品質 | 68 | 74 | 74 | 小規模で型検査・Lintあり。巨大な単一UIモジュールとJSDoc型が残る |
| 性能・拡張性 | 18 | 21 | 21 | Canvas単一描画、ページングなし。要求の100k図形/30fpsは未証明 |
| セキュリティ | 42 | 58 | 61 | JWT fail-closed、RBAC、CSP、サイズ制限、公開属性、CORS限定、監査追記専用DBトリガー(0005)。案件RBAC・WAF・SIEMなし |
| 可用性・バックアップ | 20 | 38 | 38 | 復旧スクリプトと空DB復旧試験を追加。本番自動バックアップ/RTO試験なし |
| 監視・障害対応 | 18 | 30 | 34 | request ID、5xxログ、公開health(status/version/timestamp/DB異常503)追加。アラート、SLO、当番、5分合成監視なし |
| テスト | 67 | 73 | 74 | 43 Unit/API、24 E2E、実Neon原子更新と0005トリガー拒否確認。性能・権限マトリクス・障害注入不足 |
| CI/CD・リリース | 72 | 79 | 79 | Gate、Migration、Gitleaks、復旧Job、Preview/Production。手動承認保護とActions自動deployは権限待ち(Issue #9) |
| 運用保守性 | 32 | 49 | 51 | runbook、復旧手順、改善台帳、監査CSV exportを追加。600名運用の管理UIと一次窓口なし |
| 文書 | 65 | 74 | 76 | README/API/試験/運用/評価/Roundログを更新。利用者手順とデータ辞書は不足 |
| 費用対効果 | 48 | 54 | 54 | 小規模Web構成は安価。CADエンジン再実装コストが高く、当面は併用が合理的 |
| 競合代替性 | 18 | 21 | 21 | 基本作図のみ。DWG忠実性、印刷、API拡張、オフライン、サポートで大差 |
| **総合** | **41.5** | **48.3** | **49.0** | **PoC** |
...
| 高 | 自動監視と通知がない | 障害発見が利用者申告依存になる。healthの503検知受け口は整備済みだが、5分合成監視・Teams/メール通知は未設定 |
...
8/27の監査追記専用化・CSV export・health強化はセキュリティ・監査と運用保守性のカバーを上げたが、代替率の大宗(主要業務フロー・必須機能)はDWG往復と2D中核機能に依存するため加重合計は32%で横ばい。**代替率を上げるにはPhase 1(DWG/DXF往復、寸法/hatch/block/layout/PDF)が必須**である。
...
- Phase 0 重大問題・セキュリティ: 公開/更新境界、原子更新、CSP、復旧ドリル、監査追記専用化、health監視受け口は実装済み。残るはSSO再構成(Issue #5)、案件RBAC(Issue #5)、自動デプロイ経路(Issue #9)、5分合成監視・通知(Issue #8)。
- Phase 1 中核業務完成: DWG/DXF往復(Issue #6)、寸法、ハッチ、block、layout/PDF、案件/図面一覧、版比較、UAT。
