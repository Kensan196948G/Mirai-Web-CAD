...
開発用サンプル7図面の受入・検証結果は[2026-09-05サンプル受入記録](sample-intake-2026-09-05.md)を参照。実案件100図面の認定とは分けて管理する。

追加提供された合成100件の[生成・検証結果](synthetic-intake-2026-09-05.md)も別管理。生成100/100とMirai限定往復31/100を混同しない。

アプリ側の[DXF取込ガード](dxf-import-preflight.md)は未対応Entityを含むファイルの部分取込を拒否する。完全往復対応ではない。
