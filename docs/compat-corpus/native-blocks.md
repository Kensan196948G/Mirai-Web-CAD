...
- 基点、入れ子、位置、回転、鏡像を含む符号付き非一様XYZ尺度、順序付きATTDEF/ATTRIBを扱う。
- 入れ子の回転・非一様尺度で発生するせん断を行列合成し、Canvas・範囲・交差選択・計測・EXPLODEへ反映する。変形された円/円弧は楕円/楕円弧、文字は幅係数・傾斜・反転情報として扱う。
...
- INSERTの移動・回転・符号付き非一様尺度変更と既存ATTRIBの値変更は、原本の対象group値のみを更新する。HEADER/TABLES/BLOCKS/OBJECTS等、ハンドル、所有者リンク、未使用定義、改行を保持する。
- 既存レイヤー内の対応図形の追加・削除・座標変更、既存BLOCK内の対応図形変更、通常BLOCK定義の追加・改名・削除を原本へ局所反映する。新規定義では衝突しないハンドルを採番し、BLOCK_RECORD件数と所有者を更新する。
- INSERTを含まない通常の対応DXFも、空図面への単独取込では原本とレコード対応を保存し、同じ局所編集経路を使用する。
...
- MINSERT配列、3D/傾斜OCS、XREF、幅・bulge付きポリライン、複雑な文字整列は拒否する。
- 属性は単一行。重複タグは順序付きで編集する。属性のMTEXT/複雑な整列等は拒否する。
- 属性付きEXPLODEは未対応。通常BLOCKを新規作成する既存UI/コマンドは旧children型のまま。
...
- 新規図形は各空間/定義の末尾追加に限定し、既存レコードの並替えは対象外。新規レイヤー、レイヤー属性、単位/レイアウト、複数原本の合成、MINSERT等の未対応Entity変更は警告付き限定再生成となり、完全保持ではない。
- 原本保持には今回追加した原本レコード対応情報が必要。旧保存図面は原本DXFの再取込が必要になる場合がある。
- 鏡像・非一様尺度の取込、Canvas描画、範囲/交差選択、MIRROR、EXPLODE、DXF再生成、原本保持書出しに対応した。外部CADでの目視受入は残る。
...
合成BLOCK10件・属性10件の全20件が限定取込・書出し可能。
20件の未編集/編集後、計40出力を別実装ezdxfで元DXFと比較した。
比較対象は形状、基点、入れ子参照、INSERTの位置/回転/XYZ尺度、順序付き属性の値/位置/文字高さ/回転/フラグ、単位。
編集試験では移動・回転・拡縮と先頭属性値の変更を同時に適用し、元DXFへのezdxf変換結果と照合する。
40/40で一致し、ezdxf監査のエラー・自動修復は0件。
加えて未編集20件の原本完全一致と、編集前後40件の全非ENTITIESセクションのタグ列一致を検証する。フォント/色/線種/OBJECTS等の原本データ保持は含むが、その描画や連想更新の受入は含めない。
新規BLOCK定義・符号付き非一様INSERTを原本へ追加する検証DXFもezdxf監査でエラー・自動修復0件を確認する。
...
原本保全100/100、今回の限定比較40/40、完全互換認定は混同しない。
